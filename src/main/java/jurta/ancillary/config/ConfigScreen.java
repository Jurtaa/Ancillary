...
public class ConfigScreen extends Screen {
    ...
    public ConfigScreen(Screen parentScreen) {
        ...
        super(new TranslationTextComponent("config.ancillary.title", Ancillary.MOD_ID));
        this.parentScreen = parentScreen;
    }

    @Override
    protected void init() {
        this.addButton(new Button(this.width / 2 - 75, this.height / 6 - 6, 150, 20, new TranslationTextComponent("config.ancillary.generation"), (generation) -> {
            this.minecraft.setScreen(new GenerationConfigScreen(this));
        }, (tooltip, matrixStack, mouseX, mouseY) -> {
            this.renderTooltip(matrixStack, new TranslationTextComponent("config.ancillary.generation.tooltip"), mouseX, mouseY);
        }));
        // Add the "Done" button
